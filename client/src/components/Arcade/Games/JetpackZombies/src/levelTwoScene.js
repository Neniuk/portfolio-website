...
export default class LevelTwoScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet("hallowedGround", "assets/HallowedGround.png", {
            frameWidth: 227,
            frameHeight: 131
        });
        ...
        this.load.image("item", "assets/Blood.png");
        this.load.image("hallowedItem", "assets/discoball.png");
        this.load.image("hallowedShot", "assets/Projectile.png");
        this.load.image("darkHole", "assets/dark-hole.png");
        this.load.image("enemy", "assets/zombie.png");
        this.load.image("jetpackEnemy", "assets/jetpack-zombie.png");
    ...
    create() {
        ...
        // Speech text
        this.welcomeText = this.add.text(0 + 47, this.game.config.height - 76, "You can move using either <WASD> or the <ARROW> keys. \nNow aim at those jetpack zombies with your <MOUSE> \nand press <SPACE> to shoot.", {
            fontFamily: "Arial",
            fontSize: 12,
            color: "#000000"
        });
        ...
        this.time.delayedCall(10000, () => {
            this.welcomeText.setText("The middle of the floor will fall out in 5 seconds. \nWatch out!");
        }, null, this);
        ...
        this.spawnEnemy = function(delta) {
            if (this.enemyTimer <= 0) {
                ...
                let enemy = this.physics.add.sprite(randomX, randomY, "jetpackEnemy");
                //enemy.setOrigin(0, 0);
                enemy.setDepth(3);
        ...
        this.spawnBoss = () => {
            ...
            // enemy.setRotation(targetAngle);
            this.physics.moveTo(enemy, this.player.x, this.player.y, enemySpeed * 0.75);
        };
        ...
        this.shootHallowedShot = () => {
            ...
            let hallowedShot1 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot1);
            this.hallowedProjectiles.add(hallowedShot1);
            let hallowedShot2 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot2);
            this.hallowedProjectiles.add(hallowedShot2);
            let hallowedShot3 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot3);
            this.hallowedProjectiles.add(hallowedShot3);
            let hallowedShot4 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot4);
            this.hallowedProjectiles.add(hallowedShot4);
            let hallowedShot5 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot5);
            this.hallowedProjectiles.add(hallowedShot5);
            let hallowedShot6 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot6);
            this.hallowedProjectiles.add(hallowedShot6);
            let hallowedShot7 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
            tempGroup.add(hallowedShot7);
            this.hallowedProjectiles.add(hallowedShot7);
            let hallowedShot8 = this.physics.add.sprite(this.player.x, this.player.y, "hallowedShot");
        ...
        this.spawnHallowedItem = (enemyX, enemyY) => {
            let hallowedItem = this.physics.add.sprite(enemyX, enemyY, "hallowedItem");
            this.hallowedItems.add(hallowedItem);
            console.log("Holy item spawned");
            hallowedItem.setDepth(3);

            // Reference: https://stackoverflow.com/questions/54630495/phaser-how-to-use-a-simple-timer-from-0-to-3
            this.time.delayedCall(5000, () => { hallowedItem.destroy(); }, null, this);
        };

        this.bossChargeText = this.add.text(10, 50, "Boss spawn (DESTROY ENEMIES): 0/20", {
            fontFamily: "Arial",
            fontSize: 14,
            color: "#ffffff"
        });
        ...
        this.floorsFall = () => {
            ...
            let darkhole = this.physics.add.sprite(this.game.config.width / 2, this.game.config.height / 2, "darkHole");
            //darkhole.setDepth(1);
        ...
        this.physics.add.overlap(this.items, this.darkholes, (item, darkhole) => {
            if (item.x > darkhole.x - 103 && item.x < darkhole.x + 103 && item.y > darkhole.y - 52 && item.y < darkhole.y + 52) {
                item.destroy();
            }
        });

        this.physics.add.overlap(this.hallowedItems, this.darkholes, (hallowedItem, darkhole) => {
            if (hallowedItem.x > darkhole.x - 103 && hallowedItem.x < darkhole.x + 103 && hallowedItem.y > darkhole.y - 52 && hallowedItem.y < darkhole.y + 52) {
        ...
        this.physics.add.overlap(this.hallowedProjectiles, this.enemies, (hallowedProjectile, enemy) => {
        ...
        this.physics.add.overlap(this.player, this.darkholes, (player, darkhole) => {
            // If player sprite is over the darkhole, destroy the player
            if (player.x > darkhole.x - 103 && player.x < darkhole.x + 103 && player.y > darkhole.y - 52 && player.y < darkhole.y + 52) {
                this.gameOver();
            }
            // this.gameOver();
        });
        ...
        this.physics.add.overlap(this.player, this.hallowedItems, (player, hallowedItem) => {
            hallowedItem.destroy();
            ...
            let hallowedGround = this.physics.add.sprite(this.game.config.width / 2, this.game.config.height / 2, "hallowedGround");
            
            let numOfHallowedGrounds = this.hallowedGrounds.getLength();
            
            this.hallowedGrounds.add(hallowedGround);
            console.log("This land has been blessed, by the power of epic dance moves!");
            hallowedGround.anims.play("hallowedGround");

            console.log("Hallowed grounds: " + numOfHallowedGrounds);
            if (numOfHallowedGrounds === 0) {
                this.enemies.getChildren().forEach(enemy => {
                    if (enemy.x > hallowedGround.x - 103 && enemy.x < hallowedGround.x + 103 && enemy.y > hallowedGround.y - 52 && enemy.y < hallowedGround.y + 52) {
                        enemy.destroy();
                    }
                });
        ...
        this.gameOver = () => {
            ...
            this.scene.start("MenuScene", {score: this.highscore, name: this.name});
        };
    ...
    update(time, delta) {
        ...
        this.hallowedProjectiles.getChildren().forEach(hallowedProjectile => {
            if (hallowedProjectile.x < 0 || hallowedProjectile.x > this.game.config.width || hallowedProjectile.y < 0 || hallowedProjectile.y > this.game.config.height) {
                hallowedProjectile.destroy();
        ...
        if (this.hallowedGrounds.getLength() > 0) {
            ...
            if (this.hallowedShotsLeft <= 0) {
                ...
                // Speech text
                let welcomeText = this.add.text(0 + 47, this.game.config.height - 76, "Time until next floor fall 10s!", {
                    fontFamily: "Arial",
                    fontSize: 12,
                    color: "#000000"
                });
        ...
        this.holyShotsText.setText("Holy Shots (<H>): " + this.hallowedShotsLeft);
